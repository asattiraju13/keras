  ...
  def compile(self,
    ...
    Args:
        ...
        steps_per_execution: Int. Defaults to 1. The number of batches to run
          ...
          execution).
        jit_compile: Option to compile the model's training step with XLA.
          This option cannot be enabled when run_eagerly=True.
          [XLA](https://www.tensorflow.org/xla) is an optimizing compiler for
            machine learning. Note that `jit_compile=True` is
            experimental and may not necessarily work for all models. Examples
            of unsupported operations include - SparseFillEmptyRows, _arg, etc.
            Unsupported optimizers for SimpleRNN include - RMSprop, adam,
            adadelta, adagrad, adamax, nadam, ftrl.
            For more information on supported operations please refer
            [here](https://www.tensorflow.org/xla).
          Refer to
          [XLA known issues](https://www.tensorflow.org/xla/known_issues) for
          more details.
    ...
    with self.distribute_strategy.scope():
      ...
      if (self._run_eagerly or self.dynamic) and jit_compile:
        raise ValueError(
            'Your model training configuration cannot enable '
            '`run_eagerly` and `jit_compile` at the same time.')
      else:
